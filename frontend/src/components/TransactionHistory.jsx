...
const TransactionExplorer = () => {
  ...
  return (
    <div className="bg-black text-white min-h-screen p-6">
      ...
      {/* 🌀 Loading Indicator */}
      {loading && (
        <p className="text-center text-gray-400">🌀 Fetching transactions...</p>
      )}
            ...
            {transactions.length > 0 ? (
              transactions.map((tx, index) => (
                ...
                >
                  ...
                  <td className="p-3">
                    ${((parseFloat(tx.value) / 10 ** 18) * 2000).toFixed(2)}
                  </td>
                  <td className="p-3">
                    {(parseFloat(tx.gasPrice) / 10 ** 9).toFixed(2)} GWEI
                  </td>
                  <td className="p-3">
                    <a
                      href={`https://${network}.etherscan.io/address/${tx.from}`}
                      target="_blank"
                  ...
                  <td className="p-3">
                    <a
                      href={`https://${network}.etherscan.io/tx/${tx.hash}`}
                      target="_blank"
                      className="text-blue-400"
                    >
              ...
              <tr>
                <td colSpan="6" className="text-center text-gray-400 p-3">
                  {address
                    ...
                    : "Enter an address to see transactions"}
                </td>
              </tr>
            )}
          </tbody>
