...
contract EnhancedDataRequestContract {
    ...
    // New debugging event for requests and userToRequestIds mapping
    event DebugUserRequest(
        address user,
        uint256 requestId,
        uint256[] requestIds
    );
    ...
    function requestData(address _user, string[] memory _fields) public {
        ...
        // Debugging event to track user request IDs
        emit DebugUserRequest(_user, requestIdCounter, userToRequestIds[_user]);
    }
